"""Tests for the SPECOPS_DISABLE_SSL_VERIFY toggle in specops_lib.http."""
...
from specops_lib import http as specops_lib_http
from specops_lib.http import (
    ENV_VAR,
    httpx_verify,
...
def _reset_warn_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(specops_lib_http, "_warned", False, raising=True)
...
def test_warning_is_emitted_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    calls: list[str] = []
    monkeypatch.setattr(specops_lib_http.logger, "warning", lambda msg: calls.append(msg))
