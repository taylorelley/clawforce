...
| **Plan template** | Reusable blueprint for a Plan: columns, pre-filled tasks, and optional preassigned agents (at plan and/or task level). Lives in the Marketplace (**Plan Templates** tab). Bundled starters ship in `marketplace/plan-templates/catalog.yaml`; custom entries are stored in `{storage_root}/admin/custom_plan_templates.yaml`. Applied by passing `template_id` to `POST /api/plans`. Agent ids that no longer exist are silently skipped at plan-creation time. |
