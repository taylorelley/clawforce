...
def _load_yaml_list(path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(data, list):
            logger.warning("Software catalog at %s is not a list", path)
            return []
        return data
    except (yaml.YAMLError, OSError) as e:
        logger.warning("Failed to load software catalog at %s: %s", path, e)
        return []
...
class YamlSoftwareRegistry:
    ...
    def update_custom_entry(self, software_id: str, entry: dict[str, Any]) -> bool:
        """Update an existing custom entry by id. Returns True if it was found and updated."""
        if not self._custom_catalog_path:
            return False
        existing = _load_yaml_list(self._custom_catalog_path)
        for i, e in enumerate(existing):
            if e.get("id") == software_id:
                existing[i] = {**e, **entry, "id": software_id}
                self._custom_catalog_path.write_text(
                    yaml.dump(existing, allow_unicode=True, sort_keys=False),
                    encoding="utf-8",
                )
    ...
    def delete_custom_entry(self, software_id: str) -> bool:
        """Remove a custom entry by id. Returns True if it was found and removed."""
        if not self._custom_catalog_path:
            return False
        existing = _load_yaml_list(self._custom_catalog_path)
        filtered = [e for e in existing if e.get("id") != software_id]
