...
from specops.deps import get_agent_store, get_runtime, get_skill_registry
...
def _parse_specialagent_metadata(raw: str) -> dict:
...
async def _list_skills_via_runtime(
    ...
    for name in sorted(skill_names):
        ...
        specialagent_meta = _parse_specialagent_metadata(meta.get("metadata", ""))
        emoji = specialagent_meta.get("emoji", "")
        available = _check_requirements(specialagent_meta)
        always = bool(specialagent_meta.get("always") or meta.get("always"))
        result.append(
            {
                "name": name,
