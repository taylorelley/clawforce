...
def _spawn_callback_container(
    ...
    The container runs ``specops.oauth_callback_server``, binds the port
    ...
    image = os.environ.get("AGENT_IMAGE", "ghcr.io/taylorelley/specops:latest")
    name = f"specops-oauth-cb-{state[:12]}"
    ...
    for effective_url, extra_hosts in attempts:
        ...
        kwargs: dict = {
            "image": image,
            "command": ["python", "-m", "specops.oauth_callback_server"],
            "detach": True,
            "name": name,
...
def _build_notify_url(request: Request) -> str:
    ...
    container on the same Docker host) can reach the specops server through
    ...
    # reach the specops server via the host machine's port mapping.
...
async def _run_oauth_flow(
    ...
    if container is None:
        ...
        # Works when specops is run directly on the host (not in Docker).
...
async def oauth_authorize(
    ...
    async def _bg() -> None:
        try:
            ...
            # it as SPECOPS_OPENAI_OAUTH_TOKEN — same pipeline as API keys.
            if agent_id:
