...
class AgentDef(Base):
    ...
    Defines one logical agent; the running instance is the worker (specialagent).
...
class UserDef(Base):
    """A SpecOps user account.

    role is one of the values in UserRole:
    - ``admin``: full access to every specialagent and plan, manages user accounts.
    - ``user``: sees only specialagents and plans they own or have been explicitly shared with.
    """
