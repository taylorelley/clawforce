...
Run as an ephemeral Docker container by specops during an OAuth login flow::

    docker run --rm -p 1455:1455 \\
        -e OAUTH_NOTIFY_URL=http://host.docker.internal:8080/api/providers/oauth/internal/deliver \\
        -e OAUTH_PORT=1455 \\
        specops:latest python -m specops.oauth_callback_server

Environment variables
---------------------
OAUTH_NOTIFY_URL
    The specops endpoint to POST ``{"code": "...", "state": "..."}`` to once
...
<body>
  <div class="card">
    ...
    <p>You can close this tab and return to specops.</p>
