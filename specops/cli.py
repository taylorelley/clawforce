...
from specops.core.store.users import VALID_ROLES, UserStore
from specops.deps import get_fernet

app = typer.Typer(name="specops", help="Multi-agent team admin", no_args_is_help=True)
console = Console()
...
DEFAULT_DATA_DIR = str(Path.home() / ".specops-data")
...
def agent_create(
    ...
    if show_token:
        console.print("")
        console.print("[cyan]To connect a remote specialagent:[/cyan]")
        console.print(
            f"  specialagent run --admin-url <ADMIN_URL> --agent-id {agent.id} --token {agent.agent_token}"
        )
...
def agent_token(
    ...
    console.print("[cyan]To connect a remote specialagent:[/cyan]")
    console.print(
        f"  specialagent run --admin-url {url} --agent-id {agent.id} --token {agent.agent_token}"
    )
    console.print("")
    console.print("[dim]Or set environment variables:[/dim]")
    console.print(f"  export SPECIALAGENT_CONTROL_PLANE__ADMIN_URL={url}")
    console.print(f"  export SPECIALAGENT_CONTROL_PLANE__AGENT_ID={agent.id}")
    console.print(f"  export SPECIALAGENT_CONTROL_PLANE__AGENT_TOKEN={agent.agent_token}")
...
def setup(
    ...
    """Initialize SpecOps and create the first admin user.
    ...
    console.print(f"  [cyan]specops serve --data-dir {data_dir}[/cyan]")
...
def user_list(
    ...
    users = store.list_users()
    if not users:
        console.print("[dim]No users found. Run 'specops setup' first.[/dim]")
        return
    for u in users:
...
def _user_reset_impl(username: str, data_dir: str) -> None:
    ...
    if not user:
        console.print(f"[red]User not found: {username}[/red]")
        console.print("Use 'specops user list' to see existing usernames.")
        raise typer.Exit(1)
...
def serve(
    ...
    """Start the SpecOps server (API + dashboard).

    This is the main command for running SpecOps in production.
    ...
    Examples:
        ...
        specops serve --data-dir /var/specops  # Custom data directory
        specops serve --workers 4              # Multi-process (production)
    """
    import uvicorn
    ...
    console.print("[cyan]Starting SpecOps server[/cyan]")
    console.print(
    ...
    if workers > 1 or reload:
        ...
        from specops.app import app as fastapi_app
...
def web(
    ...
    if daemon:
        ...
        cmd = [sys.executable, "-m", "specops.cli", "serve", "--port", str(port), "--host", host]
        subprocess.Popen(
...
def version():
    ...
    console.print(f"SpecOps v{__version__}")
