            ...
            <div className="flex items-center gap-2">
              <div className="relative" ref={colorRef}>
                <button type="button" onClick={() => setShowColorPicker((v) => !v)} className="shrink-0 rounded-lg p-0.5 hover:bg-claude-surface transition-colors" title="Change color">
                  <SpecialAgentIcon className="h-6 w-6" color={agent.color || undefined} />
                </button>
                {showColorPicker && (
            ...
            <div className="mt-2.5">
              <ModelProviderSection
                ...
                onProviderChange={(provider, patch) => {
                  const nextProviders: Record<string, unknown> = { ...(agent.providers ?? {}) };
                  ...
                  if (provider && (patch.apiKey !== undefined || patch.apiBase !== undefined)) {
                    const existing = (nextProviders[provider] as Record<string, unknown> | undefined) ?? {};
